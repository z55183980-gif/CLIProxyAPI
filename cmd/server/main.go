...
func main() {
	...
	flag.BoolVar(&localModel, "local-model", false, "Use the embedded models.json only, skip remote model catalog fetching")
...
// modelCatalogUpdaterPlan reports whether the remote model catalog should refresh.
// The Codex client-template updater was removed with the Codex upstream.
func modelCatalogUpdaterPlan(localModel, homeEnabled bool) (startModels bool) {
	if localModel {
		return false
	}
	return !homeEnabled
...
func startModelCatalogUpdaters(localModel, homeEnabled bool) {
	if modelCatalogUpdaterPlan(localModel, homeEnabled) {
		registry.StartModelsUpdater(context.Background())
		return
	}
	if homeEnabled {
		log.Info("Home mode: remote models.json updates disabled")
	}
}
