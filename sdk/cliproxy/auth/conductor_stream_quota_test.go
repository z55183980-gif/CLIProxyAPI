...
func TestGeminiExecuteStreamQuotaFailurePreservesCooldownAndScope(t *testing.T) {
		...
		for _, weighted := range []bool{false, true} {
			...
			t.Run(name, func(t *testing.T) {
				...
				for _, candidate := range []*Auth{
					{ID: highID, Provider: "gemini", Status: StatusActive, Attributes: map[string]string{"priority": "-4", AttributeWeight: "1"}},
					{ID: lowID, Provider: "gemini", Status: StatusActive, Attributes: map[string]string{"priority": "-3", AttributeWeight: "1"}},
				} {
					registry.GetGlobalRegistry().RegisterClient(candidate.ID, "gemini", []*registry.ModelInfo{{ID: model}, {ID: siblingModel}})
				...
				before := time.Now()
				result, errStream := manager.ExecuteStream(context.Background(), []string{"gemini"}, cliproxyexecutor.Request{Model: model}, cliproxyexecutor.Options{Stream: true})
				if errStream != nil {
				...
				for _, requestedModel := range []string{model, siblingModel} {
					...
					selected, _, _, errSelect := manager.pickNextMixed(context.Background(), []string{"gemini"}, requestedModel, cliproxyexecutor.Options{}, nil)
					if errSelect != nil {
