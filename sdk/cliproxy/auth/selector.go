...
// Lower numeric values have higher scheduling priority, matching sub2api.
...
func isAuthBlockedForModel(auth *Auth, model string, now time.Time) (bool, blockReason, time.Time) {
	...
	if auth.Unavailable && auth.LastError != nil && auth.LastError.Code == "sub2api_account_block" {
		if blocked, reason, until := availabilityBlock(true, auth.Quota.Exceeded, auth.NextRetryAfter, auth.Quota.NextRecoverAt, now); blocked {
			return blocked, reason, until
		}
	}
	if auth.Quota.Exceeded && auth.Quota.Reason == "credential_quota" && auth.Quota.NextRecoverAt.After(now) {
		return true, blockReasonCooldown, auth.Quota.NextRecoverAt
	}
	if auth.Provider == "claude" && strings.Contains(strings.ToLower(model), "fable") {
		if state := auth.ModelStates["claude-fable-5"]; state != nil {
			if blocked, reason, until := availabilityBlock(state.Unavailable, state.Quota.Exceeded, state.NextRetryAfter, state.Quota.NextRecoverAt, now); blocked {
				return blocked, reason, until
			}
		}
	}
