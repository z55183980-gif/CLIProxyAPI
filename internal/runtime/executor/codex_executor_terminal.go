...
func (codexEmptyIncompleteStreamError) IsPreOutputStreamFailure() bool { return true }
...
func codexTerminalStreamErr(eventData []byte) (statusErr, []byte, bool) {
	...
	// This status is synthesized from an SSE event, not an upstream HTTP 400.
	err.upstreamStatus = err.code
	err.streamFailure = true
	err.upstreamBody = bytes.Clone(eventData)
	return err, body, true
}
...
func newCodexStatusErr(statusCode int, body []byte) statusErr {
	...
	err.upstreamStatus = statusCode
	err.upstreamBody = originalBody
