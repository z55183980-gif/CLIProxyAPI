		...
		if responseFormat == to {
			...
			for scanner.Scan() {
				...
				if detail, ok := helps.ParseClaudeStreamUsage(line); ok {
					helps.ObserveMergedStreamUsage(&streamUsage, detail)
				}
...
func validateClaudeStreamingResponse(data []byte) error {
	...
	for scanner.Scan() {
		...
		case "error":
			...
			return statusErr{code: http.StatusBadGateway, msg: "claude executor: upstream returned error event: " + message, upstreamBody: bytes.Clone(payload), streamFailure: true}
		case "message_start":
