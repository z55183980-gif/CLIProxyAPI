...
func TestCodexTerminalQuotaFailoverByEventKind(t *testing.T) {
	for _, transport := range []string{"sse", "websocket-error", "websocket-failed"} {
		t.Run(transport, func(t *testing.T) {
			...
			const quota = `{"type":"rate_limit_exceeded","message":"You've hit your usage limit.","resets_in_seconds":3600}`
			...
			for i, id := range []string{highID, lowID} {
				...
				if _, errRegister := manager.Register(context.Background(), &cliproxyauth.Auth{
					...
					Attributes: map[string]string{"priority": fmt.Sprint(i + 1), "base_url": server.URL, "api_key": key},
			...
			if quotaErr != nil || !strings.Contains(string(payload), "response.completed") {
				t.Fatalf("preamble quota failure must fail over: payload=%s err=%v", payload, quotaErr)
			}
			...
			wantAttempts := []string{"quota-high", "quota-low", "quota-low"}
